...
public class FrameGenerationEffect extends Effect {
        ...
        if (sequence == 0) {
            ...
            if (hasFirstFrame && hasSecondFrame &&
                ...
                if (disVulkanReady && hasFirstFrame && hasSecondFrame) {
                    // DIS renders the whole generated frame at this blend factor; the effect only
                    // has to draw it. The flow was computed when the real frame arrived.
    ...
    public void setupShaderUniforms() {
        ...
        if (uUseDISFrameLoc != -1) GLES20.glUniform1i(uUseDISFrameLoc, 0);

        // A frame generated by DIS is drawn as it is (unit 4).
        if (disVulkanReady && disFrameReady) {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE4);
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, disVulkan.getOutGlTexture());
            GLES20.glUniform1i(uMotionTextureLoc, 4);
            if (uUseDISFrameLoc != -1 && isEnabled && currentSequence == 1) {
                GLES20.glUniform1i(uUseDISFrameLoc, 1);
            }
