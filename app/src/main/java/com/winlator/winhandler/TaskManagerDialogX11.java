...
public class TaskManagerDialogX11 extends ContentDialog implements OnGetProcessInfoListener {
    private final X11Activity activity;
    private final LayoutInflater inflater;
    private Timer timer;
    private final Object lock = new Object();

    public TaskManagerDialogX11(X11Activity activity) {
        ...
        cancelButton.setOnClickListener((v) -> {
            dismiss();
            ContentDialog.prompt(activity, R.string.new_task, "taskmgr.exe", (command) -> activity.getWinHandlerX11().exec(command));
        });
        ...
        setOnDismissListener((dialog) -> {
            ...
            activity.getWinHandlerX11().setOnGetProcessInfoListener(null);
        });
    ...
    private void update() {
        synchronized (lock) {
            activity.getWinHandlerX11().listProcesses();
    ...
    private void showListItemMenu(final View anchorView, final ProcessInfo processInfo) {
        ...
        listItemMenu.setOnMenuItemClickListener((menuItem) -> {
            ...
            final WinHandlerX11 winHandler = activity.getWinHandlerX11();
    ...
    private void showProcessorAffinityDialog(final ProcessInfo processInfo) {
        ...
        dialog.setOnConfirmCallback(() -> {
            WinHandlerX11 winHandler = activity.getWinHandlerX11();
    ...
    public void show() {
        update();
        activity.getWinHandlerX11().setOnGetProcessInfoListener(this);

        timer = new Timer();
        timer.schedule(new TimerTask() {
            @Override
            public void run() {
                activity.runOnUiThread(TaskManagerDialogX11.this::update);
            }
        }, 0, 1000);
        super.show();
    }
