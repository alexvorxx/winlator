...
void vkr_dis_generate_at(VkrDis* d, VkCommandBuffer cmd, float t, VkImage target_image,
                         uint32_t width, uint32_t height) {
    if (!d || !d->built || d->unavailable || d->frame_count < 2) return;
    if (t < 0.0f) t = 0.0f;
    if (t > 1.0f) t = 1.0f;
    dis_render_into(d, cmd, t, d->debug_flow ? 1 : 0, target_image, width, height, VK_NULL_HANDLE);
}
